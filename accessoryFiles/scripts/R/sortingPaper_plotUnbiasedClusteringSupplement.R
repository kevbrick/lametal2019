...
## Begin
eData <- read.table(paste0(datadir,'/k4me3Table.forR.tab'),header=TRUE)
...
#stageNames <- paste0(c("LE","ZY","EP","LP","DI"),".R1.H3K4me3.KM")
stageNames <- paste0(c("LE","ZY","EP","LP","DI"),".R1.H3K4me3.NCISTSS")
...
################################################3
panelA <- gPie + ggtitle('H3K4me3 peaks by type') + 
...
png(getIMGname(fname = 'Figure_S6',
               type = 'PNG',
               saveLocation = imgdir),
    width=7,
    height=4,
...
pdf(getIMGname(fname = 'Figure_S6',
               type = 'PDF',
               saveLocation = imgdir),
    width=7,
    height=4)
